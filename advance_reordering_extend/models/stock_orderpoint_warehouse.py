            ...
            query = f"""update stock_warehouse_orderpoint as swo
                  ...
                  from(
                          ...
                          where prod.active = true
                                ...
                                and wo.id in {tuple(rec.ids) if len(rec.ids) > 1 else '(' + str(rec.id) + ')'}
                  )tmp
                  where swo.id = tmp.id"""
            self._cr.execute(query)

            if rec.ads_qty < 1 and rec.product_min_qty == 0 and rec.product_max_qty == 0 and rec.reordering_slab.id and rec.product_id.standard_price > 0:
                line = rec.reordering_slab.reordering_slab_lines.filtered(lambda r:rec.product_id.standard_price >= r.min_cost and rec.product_id.standard_price <= r.max_cost)
                product_min_qty = line.min_inventory
                product_max_qty = line.max_inventory
                if product_min_qty >  0 and product_max_qty > 0:
                    rec.product_min_qty = product_min_qty
                    rec.product_max_qty = product_max_qty
