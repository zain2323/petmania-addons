...
class SaleOrder(models.Model):
    ...
    def create_woo_sale_order_lines(self, queue_line, order_data, tax_included, common_log_book_id, woo_taxes):
        ...
        for order_line in order_data.get("line_items"):
            taxes = []
            woo_product, odoo_product = self.find_or_create_woo_product(queue_line, order_line, common_log_book_id)
    ...
    def woo_change_shipping_partner(self, order_data, woo_instance, queue_line, common_log_book_id):
        ...
        updated_partner, updated_billing_partner, updated_shipping_partner = \
            self.woo_order_billing_shipping_partner(order_data, woo_instance, queue_line, common_log_book_id)
    ...
    def update_woo_order(self, queue_lines, log_book):
        ...
        for queue_line in queue_lines:
            ...
            try:
                ...
                if woo_status != "cancelled" and woo_instance.woo_customer_order_webhook:
                    order.woo_change_shipping_partner(order_data, woo_instance, queue_line, log_book)
                ...
                if woo_status == "cancelled" and order.state != "cancel":
                    ...
                    if woo_instance.woo_ship_order_webhook:
                ...
                if financial_status == "paid" and not refund_data:
                    ...
                    workflow = self.create_update_payment_gateway_and_workflow(order_data, woo_instance, log_book,
                                                                               queue_line)
                    if workflow:
                ...
                if refund_data and woo_instance.woo_refund_order_webhook:
                ...
                if order_woo_status and order_woo_status in ["pending", "on-hold"]:
                    if woo_instance.woo_update_qty_order_webhook:
                        order.update_qty_in_order_from_webhook(woo_instance, queue_line, order_data, log_book)

                    if woo_instance.woo_add_new_product_order_webhook:
                        order.add_new_product_in_order_from_webhook(woo_instance, queue_line, order_data, log_book)

                    if woo_instance.woo_remove_product_order_webhook:
                        order.remove_product_in_order_from_webhook(woo_instance, queue_line, order_data, log_book)

                orders.append(order)
                if message:
                    log_line = order.create_woo_log_lines(message, log_book, queue_line)
    ...
    def remove_product_in_order_from_webhook(self, instance, queue_line, order_data, log_book):
        need_to_update_line = []
        message = ""
        ...
        if response_order_line_ids:
            ...
            for need_to_remove_line in need_to_remove_lines:
                if need_to_remove_line.qty_delivered == 0:
                    ...
                    self.create_woo_log_lines(message, log_book, queue_line)
                    queue_line.write({'state': 'failed', 'processed_at': datetime.now()})
    ...
    def add_new_product_in_order_from_webhook(self, instance, queue_line, order_data, log_book):
        ...
        if need_to_create_line:
            ...
            if instance.apply_tax == "create_woo_tax":
                woo_taxes = self.woo_prepare_tax_data(order_data.get('tax_lines'), "", {}, queue_line, log_book,
                                                      instance, order_data)
            order_lines = self.create_woo_sale_order_lines_from_webhook(need_to_create_line, queue_line, order_data,
                                                                        tax_included, instance, woo_taxes, log_book)
            self.webhook_call_auto_invoice_workflow(self.auto_workflow_process_id)
    ...
    def create_woo_sale_order_lines_from_webhook(self, need_to_create_line, queue_line, order_data, tax_included,
                                                 instance, woo_taxes, log_book):
        """
        """
        order_lines_list = []
        for order_line in need_to_create_line:
            taxes = []
            woo_product, odoo_product = self.find_or_create_woo_product(queue_line, order_line, log_book)
            if not woo_product and not odoo_product:
                message = "Product [%s][%s] not found for Order %s" % (
                    order_line.get("sku"), order_line.get("name"), order_data.get('number'))
    ...
    def update_qty_in_order_from_webhook(self, instance, queue_line, order_data, log_book):
        message = ""
        updated_data = []
            ...
            for refund in pending_refund:
                ...
                for refund_line_items in new_refund_data.get('line_items'):
                    ...
                    for refund_meta_data_value in refund_meta_data_values:
                        new_invoice_line_ids = new_move.invoice_line_ids.filtered(
                            lambda x: x.sale_line_ids.woo_line_id == refund_meta_data_value.get('value'))
                        need_to_add_invoice_line_ids.append(new_invoice_line_ids.id)
                        if refund_line_items.get('quantity') == 0:
                            new_invoice_line_ids.quantity = 1
                            new_invoice_line_ids.price_unit = abs(float(refund_line_items.get("total", 0)))
                        else:
                            new_invoice_line_ids.quantity = abs(refund_line_items.get('quantity'))
