...
class WooProcessImportExport(models.TransientModel):
    ...
    def import_specific_product(self):
        ...
        if product_res_list:
            product_queues = self.env['woo.product.template.ept'].with_context(
                import_export_record=self.id, queue_created_by="manual").create_woo_product_queue(product_res_list,
    ...
    def import_specific_orders(self):
        ...
        @task ID : 197960
        """
        order_queues = self.env['sale.order'].with_context(queue_created_by="manual").import_woo_specific_orders(
            self.woo_instance_id,
    ...
    def update_stock_in_woo(self):
        ...
        """
        instance = self.woo_instance_id
        queues = self.env['woo.product.template.ept'].with_context(queue_created_by="manual").update_stock(instance,
    ...
    def get_products_from_woo(self):
        ...
        product_queues = woo_products_template_obj.with_context(
            import_export_record=self.id, queue_created_by="manual").get_products_from_woo_v1_v2_v3(woo_instance_id,
                                                                                                    woo_common_log_id,
    ...
    def import_sale_orders(self, order_type=""):
        ...
        Migrated by Maulik Barad on Date 07-Oct-2021.
        """
        order_queues = self.env['sale.order'].with_context(queue_created_by="manual").import_woo_orders(
            self.woo_instance_id, self.orders_after_date,
