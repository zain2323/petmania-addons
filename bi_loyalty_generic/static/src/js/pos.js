odoo.define('bi_loyalty_generic.pos', function(require) {
	...
	models.load_fields('res.partner', ['loyalty_pts','loyalty_amount']);
	...
	models.Order = models.Order.extend({
		...
		remove_orderline: function(line) {
			...
			if(line.id ==this.get('remove_line'))
			{
				...
				if (partner) {
					partner.loyalty_pts = parseInt(partner.loyalty_pts) + parseInt(this.get('redeem_point')) ;
				}
			}
			else
			{
			...
			{	
			   if (this.pos.pos_loyalty_setting[0].loyalty_basis_on == 'loyalty_category') {
					...
					if (order && partner_id)
					{
						...
						final_loyalty += parseInt(amount_total / loyaly_points);
						loyalty_total = order.get_client().loyalty_pts + final_loyalty;
						return Math.floor(final_loyalty);
					}
				}
			}
