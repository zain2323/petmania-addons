        ...
        for data in stock_data:
            ...
            if float(data.get('ads')) > 0:
                if float(data.get('incoming')) == 0:
                    projected_end_date = datetime.now() + (timedelta(days=float(data.get('qty_available'))) / float(data.get('ads'))) if data.get('qty_available') else ""
                else:
                    projected_end_date = datetime.now() + (timedelta(days=float(data.get('forecasted_stock'))) / float(data.get('ads'))) if data.get('forecasted_stock') else ""
                if projected_end_date != "":
                    projected_end_date = projected_end_date.date()
...
class SetuInventoryOutOfStockBIReport(models.TransientModel):
    ...
    projected_end_date = fields.Date(string="Projected End Date")
