...
class SetuInventoryXYZAnalysisReport(models.TransientModel):
    ...
    def download_report_in_listview(self):
        ...
        for fsn_data_value in stock_data:
            ...
            product = self.env['product.product'].search([('id', '=', fsn_data_value.get('product_id',''))])
            if product:
    ...
    def write_report_data_header(self, workbook, worksheet, row):
        ...
        worksheet.write(row, 5, 'Current Stock', odd_normal_right_format)
        worksheet.write(row, 6, 'Stock Value', even_normal_right_format)
        worksheet.write(row, 7, 'Stock Value (%)', odd_normal_right_format)
        worksheet.write(row, 8, 'Cumulative (%)', even_normal_right_format)
        worksheet.write(row, 9, 'XYZ Classification', odd_normal_right_format)
    ...
    def write_data_to_worksheet(self, workbook, worksheet, data, row):
        ...
        worksheet.write(row, 5, data.get('current_stock',''), odd_normal_right_format)
        worksheet.write(row, 6, data.get('stock_value',''), even_normal_right_format)
        worksheet.write(row, 7, data.get('stock_value_per',''), odd_normal_right_format)
        worksheet.write(row, 8, data.get('cum_stock_value_per',''), even_normal_right_format)
        worksheet.write(row, 9, data.get('analysis_category',''), odoo_normal_center_format)
        return worksheet
