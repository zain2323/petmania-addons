...
class BlanketOrder(models.Model):
    ...
    def action_view_sale_blanket_order_line(self):
        action = self.env.ref(
            "sale_blanket_order[3].act_open_sale_blanket_order_lines_view_tree"
        ).read()[0]
...
class BlanketOrderLine(models.Model):
    ...
    def _compute_orignal_amount(self):
        for rec in self:
            rec.orignal_amount = rec.original_uom_qty * rec.price_unit
    ...
    def _compute_ordered_amount(self):
        for rec in self:
            rec.ordered_amount = rec.ordered_uom_qty * rec.price_unit
    ...
    def _compute_discount_amount(self):
        for rec in self:
            if rec.discount:
                rec.discount_amount = ((rec.discount / 100) * rec.price_subtotal)
            else:
