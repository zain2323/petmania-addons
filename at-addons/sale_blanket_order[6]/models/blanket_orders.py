...
class BlanketOrder(models.Model):
    ...
    def open_wizard(self):
        ...
        return {
            ...
            'view_type': 'form',
            'view_mode': 'form',
            'view_id': self.env.ref('sale_blanket_order[6].discount_wizard').id,
    ...
    validity_date = fields.Date(string="Validity Date", readonly=True, default=datetime.today() + timedelta(days=30), states={"draft": [("readonly", False)]})
    quotation_date = fields.Date(string="Quotation Date", readonly=True, default=datetime.today(), states={"draft": [("readonly", False)]})
    ...
    sale_order_references = fields.Char(string="SO References", compute="_compute_so_references", default="NIL")
    

    def _compute_so_references(self):
        for rec in self:
            rec.sale_order_references = 'NIL'
            for order in rec.line_ids.sale_lines.order_id:
                if rec.sale_order_references != "NIL":
                    if order.name not in rec.sale_order_references:
                        rec.sale_order_references += ", " + order.name
                    else:
                        rec.sale_order_references = "NIL"
                else:
                    rec.sale_order_references = order.name
    ...
    def action_view_sale_blanket_order_line(self):
        action = self.env.ref(
            "sale_blanket_order[6].act_open_sale_blanket_order_lines_view_tree"
        ).read()[0]
...
class DiscountWizard(models.TransientModel):
    ...
    discount = fields.Float(
        string="Disount (%)", default=0
    )
    def action_done(self):
        records = self.env['sale.blanket.order'].browse(self.env.context.get('active_ids'))
        for rec in records:
            for line in rec.line_ids:
                rec.line_ids.write({'discount': self.discount, 'order_id': rec.id})
...
class BlanketOrderLine(models.Model):
    ...
    def create(self, values):
        new_record = super(BlanketOrderLine, self).create(values)
        new_record.price_unit = new_record._get_display_price(new_record.product_id)
        return new_record
    ...
    remaining_qty_reason = fields.Selection(string="Remaining Qty Reason", 
            selection=[
            ("product_out_of_stock", "Product Out Of Stock"),
            ("customer_payment_issue_credit_limit", "Customer Payment Issue Or Credit Limit"),
            ("limited_stock_availability", "Limited Stock Availibility"),
            ("order_reduction_by_client", "Order Reduction By Client"),
            ("upsales_by_customer_care", "Upsales By Customer Care"),
            ],
            readonly="1")
    delivered_uom_qty = fields.Float(
    ...
    orignal_amount = fields.Monetary(compute="_compute_orignal_amount", string="Orignal Amount", store=True)
    ordered_amount = fields.Monetary(compute="_compute_ordered_amount", string="Ordered Amount", store=True)
    ...
    def _compute_orignal_amount(self):
        for rec in self:
            rec.orignal_amount = (rec.original_uom_qty * rec.price_unit) - rec.discount_amount
    ...
    def _compute_ordered_amount(self):
        for rec in self:
            if rec.ordered_uom_qty > 0:
                rec.ordered_amount = (rec.ordered_uom_qty * rec.price_unit) - (rec.ordered_uom_qty * (rec.discount_amount / rec.original_uom_qty))
            else:
                rec.ordered_amount = 0
    @api.depends(
        "original_uom_qty",
        "price_unit",
        "taxes_id",
    ...
    def _compute_discount_amount(self):
        for rec in self:
            if rec.discount:
                rec.discount_amount = ((rec.discount / 100) * (rec.original_uom_qty * rec.price_unit))
            else:
