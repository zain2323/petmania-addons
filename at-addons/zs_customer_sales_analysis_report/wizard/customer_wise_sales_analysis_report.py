...
class CustomerWiseSalesAnalysisReport(models.TransientModel):
    ...
    def _get_data_for_numeric_coverage(self):
        ...
        sales_data = {}
        for invoice in invoices:
            customer_id = invoice.partner_id.name if invoice.partner_id else None
            city = invoice.partner_id.city if invoice.partner_id.city else "Undefined City"
            ...
            if not lines:
                continue
            if city not in sales_data:
                sales_data[city] = {}
            for line in lines:
                brand_name = line.product_id.product_brand_id.name if line.product_id.product_brand_id else 'Undefined Brand'
                if brand_name not in sales_data[city]:
                    sales_data[city][brand_name] = set()

                sales_data[city][brand_name].add(customer_id)

                all_brands.add(brand_name)
        # taking count of customers
        for city, brands in sales_data.items():
            for brand, customers in brands.items():
                sales_data[city][brand] = len(customers)
        return sales_data, all_brands
    ...
    def generate_numeric_coverage(self):
        ...
        # Write data rows for each customer
        row = 5
        for city, brands_data in sales_data.items():
            worksheet.write(row, 0, city, data_format)

            for brand, sales in brands_data.items():
