...
class SetuInventoryOutOfStockReport(models.TransientModel):
    ...
    def write_data_to_worksheet(self, workbook, worksheet, data, row):
        ...
        if float(data.get('ads')) > 0:
            if float(data.get('incoming')) == 0:
                projected_end_date = datetime.now() + (timedelta(days=float(data.get('qty_available'))) / float(data.get('ads')))
            else:
                projected_end_date = datetime.now() + (timedelta(days=float(data.get('forecasted_stock'))) / float(data.get('ads')))
            # projected_end_date = (pytz.utc.localize(projected_end_date_utf).astimezone(pytz.timezone('Asia/Karachi'))).date()
        else:
            projected_end_date = ''
        ...
        if product:
            ...
            purchase_orders = sorted(list(filter(lambda po: po.is_shipped == False, purchase_orders)), key=lambda po: po.create_date, reverse=False)
            purchase_order = purchase_orders[0] if len(purchase_orders) > 0 else None
            arrival_date_utf = purchase_order.date_planned if purchase_order else ''
            arrival_date = (pytz.utc.localize(arrival_date_utf).astimezone(pytz.timezone('Asia/Karachi')))
        ...
        worksheet.write(row, 21, str(projected_end_date.date()), even_normal_left_format)
        return worksheet
