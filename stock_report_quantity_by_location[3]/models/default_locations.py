    ...
    def _name_search(self, name, args=None, operator='ilike', limit=100, name_get_uid=None):
        ...
        if name:
            ...
            if operator in positive_operators:
                ...
                if not product_ids:
                    product_ids = list(self._search([('barcode', 'ilike', name)] + args, limit=limit, access_rights_uid=name_get_uid))
                # if not product_ids:
                #     product_ids = list(self._search([('product_brand_id', 'ilike', name)] + args, limit=limit, access_rights_uid=name_get_uid))
